...
_shared_pool = ThreadPoolExecutor(
    max_workers=int(os.getenv("DEEPIRI_HTTP_POOL_WORKERS", "8")),
    thread_name_prefix="deepiri-http",
)
_async_client: httpx.AsyncClient | None = None
_async_client_lock = asyncio.Lock()

SESSION_CAPABILITY = "deepiri.session.bootstrap"
...
async def _shared_async_http_client() -> httpx.AsyncClient:
...
async def _async_http_request_json(
    ...
    *,
    ...
    try:
        timeout = httpx.Timeout(timeout_s, connect=min(0.4, timeout_s))
        client = await _shared_async_http_client()
...
def normalize_authorization(authorization: str | None) -> str:
...
def build_session_result(
    auth_verify: dict[str, Any], lis_health: dict[str, Any]
) -> dict[str, Any]:
    user = None
...
def session_fingerprint(authorization: str) -> dict[str, Any]:
    """Match OrganismExecutor fingerprint shape for ComputationGraph sharing."""
    return {"input": {"authorization": authorization}, "state": {}}


def bootstrap_session_sync(authorization: str | None) -> dict[str, Any]:
    """Sync bootstrap used by capability nodes (shared thread pool)."""
    ...
    verify_f = _shared_pool.submit(_verify)
    lis_f = _shared_pool.submit(_http_get_json, f"{lis_base}/health")
    return build_session_result(verify_f.result(), lis_f.result())


async def bootstrap_session_async(authorization: str | None) -> dict[str, Any]:
    """Fast cold path: async parallel hops, no ThreadPoolExecutor churn."""
    auth_base = _env_url("AUTH_SERVICE_URL", "http://auth-service:5001")
    lis_base = _env_url(
        "LANGUAGE_INTELLIGENCE_SERVICE_URL",
        "http://language-intelligence-service:5003",
    ...
    async def _verify() -> dict[str, Any]:
        ...
        return await _async_http_request_json(
            "GET",
            f"{auth_base}/auth/verify",
            headers={"Authorization": authorization},
            timeout_s=float(os.getenv("DEEPIRI_AUTH_VERIFY_TIMEOUT_S", "1.5")),
    ...
    auth_verify, lis_health = await asyncio.gather(
        _verify(),
        _async_http_request_json("GET", f"{lis_base}/health"),
    )
    return build_session_result(auth_verify, lis_health)


def warmup_deepiri_http() -> None:
    """Open keep-alives to auth/LIS/Redis so the first user isn't connection setup."""
    auth_base = _env_url("AUTH_SERVICE_URL", "http://auth-service:5001")
    lis_base = _env_url(
        "LANGUAGE_INTELLIGENCE_SERVICE_URL",
        "http://language-intelligence-service:5003",
    ...
    client = _shared_http_client()
    for url in (f"{auth_base}/health", f"{lis_base}/health"):
        try:
            client.get(url, timeout=httpx.Timeout(1.0, connect=0.4))
        except Exception:
            pass


async def warmup_deepiri_http_async() -> None:
    await asyncio.to_thread(warmup_deepiri_http)
    # Also prime the async client pool.
    auth_base = _env_url("AUTH_SERVICE_URL", "http://auth-service:5001")
    lis_base = _env_url(
        "LANGUAGE_INTELLIGENCE_SERVICE_URL",
        "http://language-intelligence-service:5003",
    ...
    try:
        ...
        await asyncio.gather(
            client.get(f"{auth_base}/health", timeout=1.0),
            client.get(f"{lis_base}/health", timeout=1.0),
...
def warmup_computation_graph(graph: Any) -> None:
    """Prime Redis client used by ComputationGraph (SET NX / GET)."""
    try:
        graph.try_acquire_flight_lock("warmup", 1)
        graph.release_flight_lock("warmup")
    except Exception:
        pass
...
class DeepiriParallelHealthNode(Node):
    ...
    def process(self, envelope):
        ...
        auth_f = _shared_pool.submit(_http_get_json, f"{auth_base}/health")
        lis_f = _shared_pool.submit(_http_get_json, f"{lis_base}/health")
...
class DeepiriSessionBootstrapNode(Node):
    ...
    def process(self, envelope):
        ...
        session = bootstrap_session_sync(str(authorization) if authorization else None)
        envelope.state["session"] = session
        envelope.state["result"] = session
        envelope.state["deepiri_report"] = {
            "useful": session["useful"],
            "required_ok": session["useful"],
            "probes": {
                "auth_verify": session["auth_verify"],
                "lis": session["lis_health"],
            },
        }
        envelope.set_next(None)
        return NodeResult(envelope=envelope, success=True)
